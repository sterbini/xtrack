...
def luminosity(
    ...
    verbose=False,
    sigma_integration=3,
    rest_mass_b1=0.93827231,
    rest_mass_b2=0.93827231,
):
    """
    ...
    if verbose:
        print(f"br_1 is {br_1}")
        print(f"br_2 is {br_2}")
    ...
    Moeller_efficiency = (
        np.sqrt(clight**2 * np.dot(diff_v, diff_v) - np.dot(cross_v, cross_v)) / clight**2 / 2
    )

    def sx1(s):
        """The sigma_x of B1, quadratic sum of betatronic and dispersive sigma"""
        return np.sqrt(
            sigma(beta(s, beta_x1, alpha_x1), epsilon_x1, betagamma_1) ** 2
    ...
    def sy1(s):
        ...
        return np.sqrt(
            sigma(beta(s, beta_y1, alpha_y1), epsilon_y1, betagamma_1) ** 2
    ...
    def sx2(s):
        ...
        return np.sqrt(
            sigma(beta(s, beta_x2, alpha_x2), epsilon_x2, betagamma_2) ** 2
    ...
    def sy2(s):
        ...
        return np.sqrt(
            sigma(beta(s, beta_y2, alpha_y2), epsilon_y2, betagamma_2) ** 2
    ...
    if crab_crossing is not None and "CC_V_x_1" in crab_crossing.keys():
        ...
        def theta_x_1(delta_z):
            ...
            return (
                ...
                / energy_tot1
                / 1e9
                * np.sin(CC_phase_x_1 + 2 * np.pi * CC_f_x_1 / clight * delta_z)
            )
        ...
        def theta_y_1(delta_z):
            return (
                ...
                / energy_tot1
                / 1e9
                * np.sin(CC_phase_y_1 + 2 * np.pi * CC_f_y_1 / clight * delta_z)
            )
        ...
        def theta_x_2(delta_z):
            return (
                ...
                / energy_tot2
                / 1e9
                * np.sin(CC_phase_x_2 + 2 * np.pi * CC_f_x_2 / clight * delta_z)
            )
        ...
        def theta_y_2(delta_z):
            return (
                ...
                / energy_tot2
                / 1e9
                * np.sin(CC_phase_y_2 + 2 * np.pi * CC_f_y_2 / clight * delta_z)
            )
        ...
        def mx1(s, t):
            ...
            return (
                x_1
                + R12_1 * theta_x_1(s - clight * t)
                + (px_1 + R22_1 * theta_x_1(s - clight * t)) * s
            )
        ...
        def my1(s, t):
            ...
            return (
                y_1
                + R34_1 * theta_y_1(s - clight * t)
                + (py_1 + R44_1 * theta_y_1(s - clight * t)) * s
            )
        ...
        def mx2(s, t):
            ...
            return (
                x_2
                + R12_2 * theta_x_2(s + clight * t)
                + (px_2 + R22_2 * theta_x_2(s + clight * t)) * s
            )
        ...
        def my2(s, t):
            ...
            return (
                y_2
                + R34_2 * theta_y_2(s + clight * t)
                + (py_2 + R44_2 * theta_y_2(s + clight * t)) * s
            )
        ...
        def mx1(s, t):
            """The mu_x of B1 as straight line"""
            # This was the old code (not correct )
            # return x_1 + px_1 * s + phi_crab_x_1 * clight * t
            # TODO: we have dx_zeta but we need dpx_zeta
            # TODO: we need to condider the RF curvature
            # TODO: let's verify with Gianni the simmetry the the t sign...
            return x_1 + px_1 * s + phi_crab_x_1 * (s - br_1 * clight * t) 
     
        def my1(s, t):
            """The mu_y of B1 as straight line"""
            # return y_1 + py_1 * s + phi_crab_y_1 * clight * t
            return y_1 + py_1 * s + phi_crab_y_1 * (s - br_1 * clight * t) 
        ...
        def mx2(s, t):
            ...
            return x_2 + px_2 * s + phi_crab_x_2 * (s + br_2 * clight * t) 

        def my2(s, t):
            """The mu_y of B2 as straight line"""
            # return y_2 + py_2 * s + phi_crab_y_2 * clight * t
            return y_2 + py_2 * s + phi_crab_y_2 * (s + br_2 * clight * t)
        
        def kernel_double_integral(s, t):
            return (
                np.exp(
                    0.5
                    * (
        ...
        integral = integrate.dblquad(
            (lambda t, s: kernel_double_integral(s, t)),
            -sigma_integration * sigma_z,
            sigma_integration * sigma_z,
...
def luminosity_from_twiss(
    ...
    twiss_b1,
    twiss_b2,
    linear_approximation_of_crabbing=True,
    f_rev=None,
    crab=None,
    sigma_integration=3,
    verbose=False,
    ...
    assert linear_approximation_of_crabbing # not implemented yet
    ...
    if verbose:
        print(f"Crab crossing: {crab_crossing}")
    if f_rev is None:
        if "T_rev0" not in twiss_b1.keys():
            raise ValueError(
                "Revolution frequency cannot be retrieved from twiss, please provide f_rev"
            )
    ...
    lumi = luminosity(
        ...
        verbose=verbose,
        sigma_integration=sigma_integration,
    )
